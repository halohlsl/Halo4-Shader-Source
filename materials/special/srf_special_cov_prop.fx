...
#include "used_float.fxh"
DECLARE_FLOAT_WITH_DEFAULT(diffuse_wrap,		"Diffuse Wrap", "", 0, 1, float(0.0));
#include "used_float.fxh"
...
#include "used_float3.fxh"
DECLARE_FLOAT_WITH_DEFAULT(reflection_intensity,		"Reflection Intensity", "", 0, 1, float(1.0));
#include "used_float.fxh"
DECLARE_FLOAT_WITH_DEFAULT(reflection_power,		"Reflection Power", "", 0, 1, float(1.0));
#include "used_float.fxh"
...
#include "used_float.fxh"
DECLARE_FLOAT_WITH_DEFAULT(reflection_albedo_mix,	"Reflection Mix Albedo", "", 0, 1, float(0.0));
#include "used_float.fxh"
...
// Samplers
	...
	shader_data.common.shaderValues.x = fresnel;	
    fresnel = pow(fresnel, front_color_power);
	fresnel_color = lerp(front_color, middle_color, fresnel);
...
// Diffuse
    float3 diffuse = 0.0f;
    calc_diffuse_lambert_wrap(diffuse, shader_data.common, normal, diffuse_wrap, true);
	float diffuse_reflection_mask = diffuse;
...
// Composite Reflection
	...
	if (AllowReflection(shader_data.common)) {
		...
		//float fresnel = pow(shader_data.common.shaderValues.x, reflection_power);
		//fresnel = lerp(1.0, fresnel, fresnel_mask_reflection);
		
		float3 rVec = reflect(view, normal);
		reflection = sampleCUBEGamma(reflection_map, rVec);

		reflection.rgb = lerp(reflection.rgb, reflection.rgb * diffuse_reflection_mask, diffuse_mask_reflection);
		
		float3 reflection_col = lerp(reflection_color, albedo.rgb, reflection_albedo_mix);
			
		reflection.rgb *= control_map_sampled.b * reflection.a * reflection_col * reflection_intensity;
		
 		out_color.rgb += reflection.rgb;		 
	}
	

	//out_color.rgb = max(diffuse, (ambient_color+diffuse)-diffuse);
