//
// File:	 srf_layered_three_height.fx
// Author:	 v-inyang
// Date:	 06/23/11
//
...
#include "next_texture.fxh"
DECLARE_SAMPLER( layer2_spMap, "Layer2 Specular", "Layer2 Specular", "shaders/default_bitmaps/bitmaps/default_diff.tif")
#include "next_texture.fxh"
...
//shared self Illum Map
#if defined(SELFILLUM)
	DECLARE_SAMPLER(layer012_selfIllumMap,		"Layered Self Illum Map", "Layered Self Illum Map", "shaders/default_bitmaps/bitmaps/default_diff.tif");
	#include "next_texture.fxh"

	DECLARE_RGB_COLOR_WITH_DEFAULT(layer0_si_color,	"Layer 0 SelfIllum Color", "", float3(0,0,0));
	#include "used_float3.fxh"
	DECLARE_FLOAT_WITH_DEFAULT(layer0_si_intensity,	"Layer 0 SelfIllum Intensity", "", 0, 1, float(1.0));
	#include "used_float.fxh"
	DECLARE_FLOAT_WITH_DEFAULT(layer0_si_amount,	"Layer 0 SelfIllum Amount", "", 0, 1, float(1.0));
	#include "used_float.fxh"

	DECLARE_RGB_COLOR_WITH_DEFAULT(layer1_si_color,	"Layer 1 SelfIllum Color", "", float3(0,0,0));
	#include "used_float3.fxh"
	DECLARE_FLOAT_WITH_DEFAULT(layer1_si_intensity,	"Layer 1 SelfIllum Intensity", "", 0, 1, float(1.0));
	#include "used_float.fxh"
	DECLARE_FLOAT_WITH_DEFAULT(layer1_si_amount,	"Layer 1 SelfIllum Amount", "", 0, 1, float(1.0));
	#include "used_float.fxh"

	DECLARE_RGB_COLOR_WITH_DEFAULT(layer2_si_color,	"Layer 2 SelfIllum Color", "", float3(0,0,0));
	#include "used_float3.fxh"
	DECLARE_FLOAT_WITH_DEFAULT(layer2_si_intensity,	"Layer 2 SelfIllum Intensity", "", 0, 1, float(1.0));
	#include "used_float.fxh"
	DECLARE_FLOAT_WITH_DEFAULT(layer2_si_amount,	"Layer 2 SelfIllum Amount", "", 0, 1, float(1.0));
	#include "used_float.fxh"
#endif
            ...
            inout s_shader_data shader_data)
{
    ...
    //----------------------------------------------
    float4 layer0_spec  = sample2DGamma(layer0_spMap, transform_texcoord(uv, layer0_spMap_transform));
    float4 layer1_spec  = sample2DGamma(layer1_spMap, transform_texcoord(uv, layer1_spMap_transform));
    float4 layer2_spec  = sample2DGamma(layer2_spMap, transform_texcoord(uv, layer2_spMap_transform));
	...
	{
		float layer0_selfIlum  = sample2DGamma(layer012_selfIllumMap, transform_texcoord(uv, layer0_coMap_transform)).r;
		float layer1_selfIlum  = sample2DGamma(layer012_selfIllumMap, transform_texcoord(uv, layer1_coMap_transform)).g;
		float layer2_selfIlum  = sample2DGamma(layer012_selfIllumMap, transform_texcoord(uv, layer2_coMap_transform)).b;

		float3 selfIllum0 = albedo.rgb * layer0_si_color * layer0_si_intensity * layer0_selfIlum;
		float3 selfIllum1 = albedo.rgb * layer1_si_color * layer1_si_intensity * layer1_selfIlum;
		float3 selfIllum2 = albedo.rgb * layer2_si_color * layer2_si_intensity * layer2_selfIlum;

		selfIllum0 = lerp(out_color.rgb * (1-layer0_selfIlum), out_color.rgb + selfIllum0, min(1, layer0_si_amount));
		selfIllum1 = lerp(out_color.rgb * (1-layer1_selfIlum), out_color.rgb + selfIllum1, min(1, layer1_si_amount));
		selfIllum2 = lerp(out_color.rgb * (1-layer2_selfIlum), out_color.rgb + selfIllum2, min(1, layer2_si_amount));

		out_color.rgb = lerp(selfIllum0, selfIllum1, layer1_mask);
		out_color.rgb = lerp(out_color.rgb, selfIllum2, layer2_mask);
	}
#endif
