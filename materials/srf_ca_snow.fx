...
#if defined(MACRO_NORMAL)
DECLARE_SAMPLER(macro_normal_map,		"Macro Normal Map", "", "shaders/default_bitmaps/bitmaps/default_normal.tif");
#include "next_texture.fxh"
DECLARE_FLOAT_WITH_DEFAULT(normal_macro_mask_influence, 	"Macro Mask Influence", "", 0, 1, float(1.0));
#include "used_float.fxh"
#endif
...
#if defined(MACRO_NORMAL)
    	float2 macro_normal_map_uv	  = transform_texcoord(uv, macro_normal_map_transform);
		float3 macro_normal_map_sample  = sample_2d_normal_approx(macro_normal_map, macro_normal_map_uv);
		normal_map_sample = (macro_normal_map_sample + normal_map_sample) / 2.0;//I don't want to composite this like the detail normal because of clamping issues, so using the average instead.
		normal_map_sample.z =  sqrt(saturate(1.0f + dot(normal_map_sample.xy, -normal_map_sample.xy)));

		maskNormal = lerp(maskNormal,normal_map_sample,normal_macro_mask_influence);
#endif
