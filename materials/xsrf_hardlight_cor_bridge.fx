...
DECLARE_FLOAT_WITH_DEFAULT(fxuv_to_second, "Fx Map UV", "", 0, 1, float(0.0));
#include "used_float.fxh"
		...
		inout s_shader_data shader_data)
{
	...
	float2 fxuv = lerp(uv, uv2, fxuv_to_second);
	
	float4 fxMapASample = sample2D(fx_map_a, transform_texcoord(fxuv, fx_map_a_transform));
	float4 fxMapBSample = sample2D(fx_map_b, transform_texcoord(fxuv, fx_map_b_transform));
	...
	// Sample base map
	float4 base = sample2D(base_map, transform_texcoord(uv, base_map_transform) + uvDistort);
	float4 circuit = sample2D(circuit_map, transform_texcoord(uv2, circuit_map_transform) + uvDistort);	
	base += circuit * circuit_amount;
	base.a += circuit.r;
	...
	float2 paletteCoord = transform_texcoord(float2(base.r, 0.0), palette_map_transform);
	float4 paletteValue = sample2DLOD(palette_map, paletteCoord, 0, true);
