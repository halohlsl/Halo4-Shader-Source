//
// File:	 srf_ca_shield.fx
// Author:	 v-jcleav
// Date:	 1/23/12
//
// Surface Shader - Shield based off of the forerunner shield used for CA's Dominion game mode
//
...
DECLARE_RGB_COLOR_WITH_DEFAULT(damageTint, "Damage Tint", "", float3(1,1,1));
#include "used_float3.fxh"
DECLARE_FLOAT_WITH_DEFAULT(damageBlendAmt, "Damage Blend Amount", "", 0, 1, float(0.0));
#include "used_float.fxh"
	...
	inout s_shader_data shaderData)
{
	...
	// For now, simply grab 1 channel, since we rely on team color
	palettizedColor.rgb = float3(palettizedColor.g, palettizedColor.g, palettizedColor.g);

	float maskMultiply = saturate((maskValue - finalMaskOffset) * finalMaskScale);

	float4 tint = ps_material_object_parameters[0];
	tint.rgb = lerp(tint, damageTint, damageBlendAmt);

	shaderData.common.albedo = palettizedColor * tint * alpha * lerp(maskMultiply, 1.0, shieldColorInput) * intensity;
}
