...
#define compute_ssao_horizontal_blur_vs		static_probe_vs				// the static_probe entry point gets used for the horizontal SSAO blur pass
#define compute_ssao_horizontal_blur_ps 	static_probe_ps
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
screen_output_ssao compute_ssao_vs(s_screen_vertex IN)
{
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
screen_output downsample_normals_vs(s_screen_vertex IN)
{
	screen_output    OUT;
	OUT.texcoord=	 IN.texcoord;
	OUT.position.xy= IN.position;
...
float filter_ssao_with_normals( float2 offset_direction, float2 texcoord )
{
	...
	float center_tap_depth_camera_space= sample2DLOD( depth_sampler, texcoord, 0, false).r;

	// Sample center tap normal:
	float3  center_tap_normal= sample2DLOD( normals_sampler, texcoord, 0, false).rgb * 2.0f - 1.0f;
    ...
    for ( int sample_index= 0; sample_index < number_of_taps; sample_index++ )
    {
		...
		float sample_depth_camera_space= sample2DLOD( depth_sampler, sample_texcoord.xy, 0, false ).r;
		...
		sample_normal= sample2DLOD( normals_sampler, sample_texcoord.xy, 0, false ).rgb * 2.0f - 1.0f;
		...
		sample_occlusion= sample2DLOD( ssao_sampler, sample_texcoord.xy, 0, false ).r;
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
screen_output compute_ssao_horizontal_blur_vs( s_screen_vertex IN )
{
	screen_output    OUT;
	OUT.texcoord=	 IN.texcoord;
	OUT.position.xy= IN.position;
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
screen_output compute_ssao_vertical_blur_vs( s_screen_vertex IN )
{
	screen_output    OUT;
	OUT.texcoord=	 IN.texcoord;
	OUT.position.xy= IN.position;
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
screen_output upsample_ssao_and_apply_vs( s_screen_vertex IN )
{
	screen_output    OUT;
	OUT.texcoord=	 IN.texcoord;
	OUT.position.xy= IN.position;
...
BEGIN_TECHNIQUE single_pass_single_probe
{
	pass screen
	{
		SET_VERTEX_SHADER(single_pass_single_probe_vs());
		SET_PIXEL_SHADER(single_pass_single_probe_ps());
	}
}
