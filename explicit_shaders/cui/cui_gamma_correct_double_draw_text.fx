...
float4 default_ps(s_screen_vertex_output input) : SV_Target
{
	...
	// Apply first pass translucent color tinting
	float4 tintColor = float4(k_texture1TintColor.rgb, k_texture1TintColor.a * k_texture1TintOpacity);
	float4 color1 = cui_tint(color, cui_linear_to_gamma2(tintColor));

	// Apply second pass additive color tinting
	tintColor = float4(k_texture2TintColor.rgb, k_texture2TintColor.a * k_texture2TintOpacity);
	float4 color2 = cui_tint(color, cui_linear_to_gamma2(tintColor));

	// Combine the first and second passes. The first pass will be translucent, and the second pass will be additive.
	color = float4(color1.rgb + color2.rgb, color1.a);
