...
// 
// ---- COLOR OUTPUTS
// k_cui_pixel_shader_color0= secondary color
// 
// ---- SCALAR OUTPUTS
...
// R: intensity
// G: blend factor for lerping between tint color and color2
// B: meter mask
...
float4 BuildResultSingle(float2 texcoord)
{
	...
	float4 result = float4(bitmap_result.rgg, bitmap_result.a);
...
float4 BuildResultMultisampled(in float2 texcoord, in float4 gradients)
{
		...
		step(sampleGroup2[3], curMeterValue) != primaryClipped)
	{
		...
		return float4(primarySample.rgg, primarySample.a * step(weightedSum, curMeterValue) * weightedAlpha);
	}

	// Fast path uses fallthrough from conditional
	return float4(primarySample.rgg, primarySample.a * primaryClipped);
}
...
float4 default_ps(s_screen_vertex_output input) : SV_Target
{
	...
	float4 tint = cui_linear_to_gamma2(lerp(k_cui_pixel_shader_tint, k_cui_pixel_shader_color0, result.g));
