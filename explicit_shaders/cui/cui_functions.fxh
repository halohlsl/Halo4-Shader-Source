#if !defined(__CUI_FUNCTIONS_FXH)
#define __CUI_FUNCTIONS_FXH
...
// Convert a non-premultiplied color into a premultiplied color
float4 cui_convert_to_premultiplied_alpha(in float4 color, in float4 color_transform)
{
	// if transform.z is 0 then we need to multiply the rgb times the alpha for this texture
	color.rgb *= max(color.a, color_transform.z);

	// we need alpha to be inverted so that 1.0 is transparent.  Multiply-add a scale and an offset.
	color.a= color.a * color_transform.x + color_transform.y;
