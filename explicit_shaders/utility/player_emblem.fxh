...
float4 calc_emblem_h4_original(
	in float2 texcoord,
...
float4 calc_emblem(
	...
	uniform bool multilayered)
{
	bool emblem_use_uui_format= (type_flags.a!=0);

	if(emblem_use_uui_format)
	{
		float4 h3_compatible_emblem_pixel = generate_h3_compatible_emblem_pixel(texcoord);
		h3_compatible_emblem_pixel.rgb = h3_compatible_emblem_pixel.rgb * h3_compatible_emblem_pixel.a;
		h3_compatible_emblem_pixel.a = (1.0 - h3_compatible_emblem_pixel.a);
		return h3_compatible_emblem_pixel;
	}
	else
	{
		return calc_emblem_h4_original(texcoord, multilayered);
	}
}
