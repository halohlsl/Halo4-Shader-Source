...
#include "render_instance_imposter_registers.fxh"
	...
	in s_imposter_vertex vertex)
{
	...
	OUT.position= mul(float4(vertex.position.xyz, 1.0f), vs_view_view_projection_matrix);

	// world space direction to eye/camera
	OUT.fragment_to_camera_world.xyz= vs_view_camera_position-vertex.position;
	OUT.fragment_to_camera_world.w=		dot(OUT.fragment_to_camera_world.xyz, vs_view_camera_backward);


	OUT.color.rgb= vertex.color.rgb * exp2(vertex.color.a * 63.75 - 31.75) * brightness.x;
	OUT.color.w= 0;
	OUT.texcoord_and_hdr_scalar= 0;
...
LOCAL_SAMPLER2D(non_cubemap_sampler, 0);


imposter_pixel render_instance_polygon_ps( s_imposter_interpolators IN ) :SV_Target0
{
	float4 out_color;
	out_color.rgb= IN.color.rgb;

	out_color.a=	0;//compute_antialias_blur_scalar(IN.fragment_to_camera_world.xyz) * (1.0f / 32.0f);
...
imposter_pixel render_instance_card_ps( s_imposter_interpolators IN ) :SV_Target0
{
...
{
	pass structure_instance_imposter
	{
