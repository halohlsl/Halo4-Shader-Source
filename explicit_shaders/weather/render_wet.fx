...
#include "deform.fxh"
#include "render_wet_registers.fxh"
...
{
	float pixel_depth= sample2D(k_ps_wet_sampler_depth_buffer, texcoord).r;
	...
	// get pixel normal
	pixel_normal= sample2D(k_ps_wet_sampler_normal_buffer, texcoord.xy).rgb;
	...
	const float game_time= k_ps_ripple_scroll_coeff.x;
	{
		...
		float3 ripple_normal_0= sample3DLOD(k_ps_wet_sampler_ripple, ripple_texcoord, 1, false).xyz;
		...
		float3 ripple_normal_1= sample3DLOD(k_ps_wet_sampler_ripple, ripple_texcoord, 1, false).xyz;
	...
	out float3 reflection_color)
{
	...
	float4 reflection_0= sampleCUBELOD(k_ps_wet_sampler_cubemap_0, reflect_dir, 0, false); 
	float4 reflection_1= sampleCUBELOD(k_ps_wet_sampler_cubemap_1, reflect_dir, 0, false); 
...
float4 screen_wet_ps( s_wet_interpolators IN ) : SV_Target0
{	
	float3 pixel_normal;
	float distance;
	float3 view_dir;
	...
	return apply_exposure(float4(output_color, wet_coeff));
}


float4 screen_fog_ps( s_wet_interpolators IN ) : SV_Target0
{	
	float3 pixel_normal;
	float distance;
	float3 view_dir;
	float3 ripple_normal;
	float pixel_depth= sample2D(k_ps_wet_sampler_depth_buffer, IN.texcoord_and_position.xy).r;
	...
	if (pixel_position.z < 0.3f && view_dir.z > 0)
	{
		...
		float3 ripple_normal= sample3DLOD(k_ps_wet_sampler_ripple, ripple_texcoord, 3).xyz;
	...
	return apply_exposure(float4(output_color, fog_coeff));
}
...
float4 screen_wet_ps(s_wet_interpolators INTERPOLATORS) :SV_Target0
{
	return float4(0,1,2,3);
}

float4 screen_fog_ps(s_wet_interpolators INTERPOLATORS) :SV_Target0
{
	return float4(0,1,2,3);
}
