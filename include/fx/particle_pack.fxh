#if !defined(__PARTICLE_PACK_FXH)
#define __PARTICLE_PACK_FXH

#if DX_VERSION == 11

#include "packed_vector.fxh"
#include "fx/raw_particle_state.fxh"

s_particle_memexported_state unpack_particle_state(in s_raw_particle_state input)
{
	float4 unpacked_rot = UnpackUShort4N(input.rot);
	float4 unpacked_time = UnpackHalf4(input.time);
	float4 unpacked_anm = UnpackHalf4(input.anm);
	float3 unpacked_anm2 = UnpackDec3N(input.anm2);
	float4 unpacked_vel = UnpackHalf4(input.vel);

	s_particle_memexported_state output;
	...
	output.m_white_point = unpacked_anm2.y;
	output.m_palette_v = unpacked_anm2.z;
	output.m_game_simulation_a = unpacked_anm.z;
	output.m_game_simulation_b = unpacked_anm.w;
	
	return output;
}

s_raw_particle_state pack_particle_state(in s_particle_memexported_state input)
{
	...
	output.anm = PackHalf4(float4(input.m_rotational_velocity, input.m_frame_velocity, input.m_game_simulation_a, input.m_game_simulation_b));
	output.anm2 = PackDec3N(float3(input.m_black_point, input.m_white_point, input.m_palette_v));
