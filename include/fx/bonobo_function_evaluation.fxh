#if !defined(__BONOBO_FUNCTION_EVALUATION_FXH)
#define __BONOBO_FUNCTION_EVALUATION_FXH
...
float evaluate_periodic_internal(int periodic_type, float input)
{
	...
	{
		output= 0.5f*(cos(2 * pi*input)+1);
	}
	else if (periodic_type==_periodic_function_cosine_with_random_period)
	{
		output= 0.5f*(cos(2 * pi*input)+1);//###davcook $TODO not yet implemented
	}
	...
	{
		output= 0.5f*(cos(2 * pi*input/4.f)*sin(2 * pi*input/2.f) + cos(2 * pi*input/7.f)*cos(4.f*2 * pi*input) +
					sin(2 * pi*input/4.f)*cos(7.f*2 * pi*input))+1;
	}
	else if (periodic_type==_periodic_function_wander)
	{
		output= 0.5f*(cos(2 * pi*input)*sin(2 * pi*input/2.f) + 0.5f*cos(2 * pi*input/7.f)*cos(4.f*2 * pi*input) +
					0.5f*sin(2 * pi*input/4.f)*cos(7.f*2 * pi*input))+1;
	}
...
float3 map_to_color_range(int cindex_min, int cindex_max, float intermediate)
{
	...
	return lerp(g_all_colors[cindex_lo], g_all_colors[cindex_hi], interp).rgb;
}
...
float2 map_to_point2d_range(int cindex_min, int cindex_max, float intermediate)
{
	...
	return lerp(g_all_colors[cindex_lo], g_all_colors[cindex_hi], interp).rg;
}
...
float3 map_to_vector3d_range(int cindex_min, int cindex_max, float intermediate)
{
	...
	return (sin((1.0f-interp)*angle)*g_all_colors[cindex_lo].rgb + sin(interp*angle)*g_all_colors[cindex_hi].rgb) / 
		sin_angle;
}
...
float2 map_to_vector2d_range(int cindex_min, int cindex_max, float intermediate)
{
	...
	//   p(t) = [sin((1-t)a)*p0 + sin(ta)*p1]/sin(a)
	// For non-unit directions, gives uniform rotation and slightly non-linear expansion.
	return (sin((1.0f-interp)*angle)*g_all_colors[cindex_lo].rg + sin(interp*angle)*g_all_colors[cindex_hi].rg) / 
		sin_angle;
}
...
#endif 	// !defined(__BONOBO_FUNCTION_EVALUATION_FXH)